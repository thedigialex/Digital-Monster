    ...
    <x-elements.container>
        <form method="POST" action="{{ isset($item) ? route('user.handleItem', [$user->id, $item->id]) : route('user.handleItem', $user->id) }}">
            ...
            <div class="mb-4 flex space-x-4">
                <div class="w-1/3">
                    <x-elements.input-label for="item_id">Item</x-elements.input-label>
                ...
                <div class="w-1/3">
                    <x-elements.input-label for="quantity">Quantity</x-elements.input-label>
                    <x-text-input id="quantity" name="quantity" type="number" value="{{ $item ? $item->quantity : old('quantity') }}" required></x-text-input>
                </div>

                <div class="w-1/3">
                    <x-elements.input-label for="is_equipped">Is Equipped</x-elements.input-label>
